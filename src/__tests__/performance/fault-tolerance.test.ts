...
import { JobStatus, JobType, JobStage } from '@prisma/client';
...
import { testTexts, mockAIExtractions, sampleTriples } from '../fixtures/test-data.js';
import { setupTestSuite, cleanupTestDatabase, waitFor } from '../helpers/test-setup.js';
...
describe('Fault Tolerance and Recovery', () => {
  ...
  describe('AI Service Failure Recovery', () => {
    it('should handle intermittent AI service failures with retry', async () => {
      ...
      mockAIProvider.generateText.mockResolvedValue(createSuccessResult({
    ...
    it('should handle circuit breaker activation under load', async () => {
      ...
      mockAIProvider.generateText.mockRejectedValue(
        new Error('AI service overloaded')
      );
    ...
    it('should recover from circuit breaker after timeout', async () => {
      ...
      mockAIProvider.generateText.mockRejectedValueOnce(
  ...
  describe('Partial Processing Failures', () => {
    ...
    it('should salvage partial results from failed extractions', async () => {
      ...
      const { extractKnowledgeTriples: mockExtract } = require('~/features/knowledge-extraction/extract.js');

      // Return partial success with some failed components
      mockExtract.mockResolvedValue(createSuccessResult({
  ...
  describe('Database Transaction Failures', () => {
    it('should handle transaction rollbacks without data corruption', async () => {
      ...
      (db.knowledgeTriple as any) = {
        ...
        createMany: jest.fn().mockRejectedValue(
