...
import { JobStatus, JobType, JobStage, type ProcessingJob } from '@prisma/client';
...
export const mockAIResponses = {
  ...
  embeddings: new Map<string, number[]>([
    ['John Smith', Array(1536).fill(0).map(() => Math.random())],
    ['software engineer', Array(1536).fill(0).map(() => Math.random())],
    ['Tech Corp', Array(1536).fill(0).map(() => Math.random())],
  ]),
};
