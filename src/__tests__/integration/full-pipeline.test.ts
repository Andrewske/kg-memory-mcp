...
import { initiateKnowledgePipeline, getPipelineStatus } from '~/features/knowledge-processing/pipeline-coordinator.js';
...
import { testTexts, mockAIExtractions, sampleTriples, sampleConcepts } from '../fixtures/test-data.js';
import { setupTestSuite, cleanupTestDatabase } from '../helpers/test-setup.js';
...
describe('Full Pipeline Integration', () => {
  ...
  describe('Pipeline Initiation and Extraction', () => {
    ...
    it('should handle text chunking in large documents', async () => {
      ...
      for (let i = 0; i < 5; i++) { // Expect multiple chunks
        mockAIProvider.generateText
          .mockResolvedValueOnce(createSuccessResult({
            data: mockAIExtractions.fourStage.entityEntity.response,
            usage: mockAIExtractions.fourStage.entityEntity.usage
