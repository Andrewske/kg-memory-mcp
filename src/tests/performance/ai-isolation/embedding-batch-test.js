		...
		const customEmbeddingService = {
			...
			embedBatch: async (batchTexts, context) => {
				...
				for (let batchIndex = 0; batchIndex < batches.length; batchIndex++) {
					...
					console.log(
						`[${testName}] Batch ${batchIndex + 1}/${batches.length}: ${batchDuration.toFixed(2)}ms (${batch.length} texts)`
					);
				}
...
async function runEmbeddingBatchTest() {
	...
	for (const setResult of results) {
		...
		for (const result of setResult.results) {
			if (!result.success) {
				console.log(
					`❌ Batch ${result.batchSize}: FAILED - ${result.error?.message || 'Unknown error'}`
				);
				continue;
			}
	...
	const sortedBatches = Object.entries(batchAnalysis).sort(([, a], [, b]) => {
	...
	if (duplicateResult.success) {
		...
		console.log(
			`   Efficiency: ${(duplicateResult.results.duplicateEfficiency * 100).toFixed(1)}%`
		);
...
function generateEmbeddingRecommendations(batchAnalysis, duplicateResult) {
	...
	const optimal = batches.reduce((a, b) => (a[1].avgEfficiency > b[1].avgEfficiency ? a : b));
	recommendations.push(
		`Optimal batch size: ${optimal[0]} (${optimal[1].avgEfficiency.toFixed(1)} texts/call, ${optimal[1].avgTokenSpeed.toFixed(1)} tok/s)`
	);
	...
	if (highEfficiencyBatches.length > 0) {
		...
		recommendations.push(
			`High-efficiency batch sizes: ${sizes} - consider using these for production`
		);
	}

	// Speed analysis
	const avgSpeed =
		batches.reduce((sum, [, analysis]) => sum + analysis.avgTokenSpeed, 0) / batches.length;
	if (avgSpeed < 1000) {
		recommendations.push(
			`Average embedding speed is ${avgSpeed.toFixed(1)} tok/s - investigate network latency and API limits`
		);
	}
	...
	if (largeBatches.length > 0) {
		const avgLargeEfficiency =
			largeBatches.reduce((sum, [, analysis]) => sum + analysis.avgEfficiency, 0) /
			largeBatches.length;
		if (avgLargeEfficiency > 90) {
			recommendations.push(
				'Large batch sizes (100+) show good efficiency - consider increasing default batch size'
			);
		} else {
			recommendations.push(
				'Large batch sizes show reduced efficiency - stick to medium batch sizes (32-64)'
			);
		}
	}
	...
	if (duplicateResult?.success && duplicateResult.results.duplicateEfficiency) {
		...
		if (efficiency < 90) {
			recommendations.push(
				`Duplicate detection efficiency is ${efficiency.toFixed(1)}% - verify embedding deduplication is working correctly`
			);
		}
	}
