/**
 * Batch extraction handler - Coordinates parallel extraction while preserving optimizations
 */
...
export class BatchExtractionJobHandler implements JobHandler {
	canHandle(jobType: JobType): boolean {
		return jobType === JobType.EXTRACT_KNOWLEDGE_BATCH || jobType === JobType.PROCESS_KNOWLEDGE;
	}
	...
	async execute(job: ProcessingJob): Promise<JobResult> {
		...
		try {
			...
			// Schedule post-processing jobs (concepts and deduplication)
	...
	private async processChunksWithResourceLimits(
		...
		const chunkPromises = chunks.map(async (chunk, index) => {
			return await semaphore.acquire(async () => {
				...
				await updateJobProgress(jobId, Math.round(progress));
