...
import { testTexts, mockAIExtractions, errorScenarios } from '../fixtures/test-data.js';
...
describe('Knowledge Extraction', () => {
  ...
  describe('extractKnowledgeTriples', () => {
    ...
    it('should extract triples using single-pass method when configured', async () => {
      ...
      mockAIProvider.generateText.mockResolvedValueOnce(createSuccessResult({
    ...
    it('should use retry mechanism for AI operations', async () => {
      ...
      (retryAIOperation as jest.Mock).mockImplementation(async (fn: any, operation: any, config: any) => {
    ...
    it('should use circuit breaker for AI operations', async () => {
      ...
      (withCircuitBreaker as jest.Mock).mockImplementation(async (fn: any, key: any, config: any) => {
    ...
    it('should track token usage correctly', async () => {
      ...
      expect(trackTokenUsage).toHaveBeenCalledWith(
        mockAIExtractions.singlePass.usage
      );
    });
    ...
    it('should handle AI provider failures gracefully', async () => {
      ...
      (retryAIOperation as jest.Mock).mockRejectedValue(
        createErrorResult('AI service unavailable', 'ai_provider')
      );

      const result = await extractKnowledgeTriples(args);

      expect(result.success).toBe(false);
      expect(result.error?.operation).toBe('ai_provider');
    });
