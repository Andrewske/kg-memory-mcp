...
describe('Embedding Cache Optimization', () => {
  ...
  describe('generateEmbeddingMap', () => {
    it('should generate embeddings for unique texts only', async () => {
      ...
      const concepts = [
        createTestConcept({ name: 'Software Engineering' }),
        createTestConcept({ name: 'Technology' })
      ];
      ...
      const expectedUniqueTexts = [
        'John Smith', 'Tech Corp', 'works at', 'Software Engineer', 'Sarah Johnson',
        'John Smith works at Tech Corp', 'John Smith works at Software Engineer', 'Sarah Johnson works at Tech Corp',
    ...
    it('should process embeddings in batches', async () => {
      ...
      const concepts = Array.from({ length: 20 }, (_, i) =>
        createTestConcept({ name: `Concept ${i}` })
      );
    ...
    it('should create comprehensive embedding map', async () => {
      const triples = [
        createTestTriple({ subject: 'John', predicate: 'likes', object: 'coffee' })
      ];
      const concepts = [
        createTestConcept({ name: 'Preferences' })
      ];

      mockEmbeddingService.generateEmbeddings.mockResolvedValue(createSuccessResult({
        embeddings: [
          [0.1, 0.2, 0.3], // John
          [0.4, 0.5, 0.6], // coffee  
          [0.7, 0.8, 0.9], // likes
    ...
    it('should preserve text-embedding relationships', async () => {
      ...
      const concepts = [
        createTestConcept({ name: 'Unique Concept' })
      ];
    ...
    it('should optimize for real-world deduplication scenarios', async () => {
      ...
      const concepts = [
        createTestConcept({ name: 'Technology' }),
        createTestConcept({ name: 'Technology' }), // Duplicate concept name
        createTestConcept({ name: 'Business' })
      ];
