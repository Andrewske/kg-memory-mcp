...
describe('Resource Limits Performance', () => {
	...
	describe('Memory and CPU Performance', () => {
		...
		it('should handle CPU-intensive operations efficiently', async () => {
			...
			const promises = Array.from({ length: operations }, () => manager.withAI(cpuIntensiveTask));
			...
			const baselineTests = [
				{
					...
					test: async () => {
						...
						await Promise.all(
							Array.from({ length: operations }, () => semaphore.acquire(async () => waitFor(1)))
						);
				...
				},
				{
					...
					test: async () => {
						...
						await Promise.all(
							Array.from({ length: operations }, () => manager.withAI(async () => waitFor(5)))
						);
