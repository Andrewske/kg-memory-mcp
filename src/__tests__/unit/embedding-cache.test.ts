...
describe('Embedding Cache Optimization', () => {
	...
	beforeEach(() => {
		...
		Object.assign(env, { BATCH_SIZE: 32 });
	});
	...
	describe('generateEmbeddingMap', () => {
		...
		it('should collect all text types for embedding', async () => {
			...
			const calledTexts = mockEmbeddingService.embedBatch.mock.calls[0][0];
		...
		it('should process embeddings in batches', async () => {
			...
			Object.assign(env, { BATCH_SIZE: 10 }); // Small batch size to force multiple batches
			...
			expect(result.success).toBe(true);
			if (result.success) {
				expect(mockEmbeddingService.embedBatch).toHaveBeenCalledTimes(
		...
		it('should handle partial batch failures', async () => {
			...
			Object.assign(env, { BATCH_SIZE: 5 });
