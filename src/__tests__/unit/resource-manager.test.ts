...
describe('Resource Manager', () => {
  ...
  describe('ResourceManager', () => {
    ...
    it('should provide accurate resource usage metrics', async () => {
      ...
      const dbOp = manager.withDB(async () => {
        await waitFor(100);  
        return 'db';
      });
    ...
    it('should handle timeout scenarios', async () => {
      ...
      // Don't wait for the long operation, just verify the quick one eventually completes
      ...
      // This should timeout because the long operation is blocking
    ...
    it('should track memory usage correctly', () => {
      ...
      expect(status.memory.usedMB).toBeLessThan(512);
      expect(typeof status.memory.usedMB).toBe('number');
    });
