...
IMPORTANT: Return ONLY a valid JSON object, no markdown formatting, no code blocks, no additional text. The response must be parseable JSON that matches the expected schema.`;
	...
	try {
		...
		try {
			...
			console.warn('[ExtractUsingTextGeneration] JSON parse failed, raw response:', result.data.data);
			return {
				success: false,
				error: {
					type: 'PARSE_ERROR',
		...
		if (parsedData?.triples && Array.isArray(parsedData.triples)) {
			parsedData.triples = parsedData.triples.filter((triple: any) => 
				triple.subject && triple.subject.trim() !== '' &&
				triple.predicate && triple.predicate.trim() !== '' &&
				triple.object && triple.object.trim() !== ''
			);
		}
		...
		if (!validation.success) {
			console.warn('[ExtractUsingTextGeneration] Schema validation failed:', validation.error.message);
...
async function extractFourStage(data: ProcessKnowledgeArgs) {
	...
	try {
				...
				if (error?.cause?.text) {
					...
					try {
						...
						if (partialData?.triples) {
							const validTriples = partialData.triples.filter((triple: any) => 
								triple.subject && triple.subject.trim() !== '' &&
								triple.predicate && triple.predicate.trim() !== '' &&
								triple.object && triple.object.trim() !== ''
							);
							if (validTriples.length > 0) {
								console.debug(`[ExtractFourStage] Salvaged ${validTriples.length} valid triples from failed ${type} extraction`);
						...
						console.debug(`[ExtractFourStage] Could not salvage triples from ${type} error: ${parseError}`);
					}
				}
				console.warn(`[ExtractFourStage] Failed to extract ${type} relationships (will continue):`, error?.message || error);
			} else if (result.status === 'fulfilled' && !result.value.success) {
				console.warn(`[ExtractFourStage] Failed to extract ${type} relationships (will continue):`, result.value.error?.message || result.value.error);
			}
		}
		...
		if (allTriples.length > 0) {
			console.warn('[ExtractFourStage] Extraction partially failed but returning partial results:', error);
			return {
				success: true,
				data: {
					triples: allTriples,
...
export async function extractByType(data: ProcessKnowledgeArgs, type: TripleType) {
	try {
		...
		const validTriples = result.data.data.triples.filter(triple => 
			triple.subject && triple.subject.trim() !== '' &&
			triple.predicate && triple.predicate.trim() !== '' &&
			triple.object && triple.object.trim() !== ''
		);
		...
		if (validTriples.length < result.data.data.triples.length) {
			console.debug(`[ExtractByType] Filtered out ${result.data.data.triples.length - validTriples.length} invalid triples with empty fields from ${type}`);
		}
