...
import { createTokenTrackingService } from "~/shared/services/token-tracking-service.js";
import { createTrackedEmbeddingService } from "~/shared/services/tracked-ai-provider.js";
...
const createDefaultConfig = (): KnowledgeGraphConfig => ({
	...
	extraction: {
		extractionMethod:
			(process.env.KG_EXTRACTION_METHOD as "single-pass" | "four-stage") ||
			"single-pass",
	...
	deduplication: {
		...
		semanticSimilarityThreshold: parseFloat(
			process.env.KG_SEMANTIC_THRESHOLD || "0.85",
		),
		exactMatchFields: ["subject", "predicate", "object", "type"],
	},
	ai: {
		provider:
			(process.env.KG_AI_PROVIDER as "openai" | "anthropic") || "openai",
...
async function main(): Promise<void> {
	try {
		...
		const baseEmbeddingService = createEmbeddingService(serverConfig.embeddings);
		const aiProvider = createAIProvider(serverConfig.ai);
		const tokenTracker = createTokenTrackingService(db);

		// Create tracked embedding service for token tracking
		const embeddingService = createTrackedEmbeddingService(
			baseEmbeddingService,
			tokenTracker,
			{
				provider: "openai", // Embedding models are OpenAI
				model: serverConfig.embeddings.model,
			},
		);
		...
		console.log(
			`📡 STDIO Transport: ${transportConfig.enableStdio ? "enabled" : "disabled"}`,
		);
		console.log(
			`🌐 HTTP Transport: ${transportConfig.enableHttp ? "enabled" : "disabled"}`,
		);
