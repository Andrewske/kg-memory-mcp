...
async function measureBasicLatency(aiProvider, iterations = 3) {
	...
	console.log(
		`   Average Latency: ${avgLatency.toFixed(2)}ms (${successfulResults.length}/${iterations} successful)`
	);
...
async function measurePromptComplexity(aiProvider) {
	...
	for (const [name, { prompt, expectedTokens }] of Object.entries(testPrompts)) {
		...
		try {
			...
			const result = await aiProvider.generateObject(prompt, flexibleSchema, {
			...
			if (result.success) {
				...
				console.log(
					`     Speed: ${tokensPerMs.toFixed(3)} tokens/ms (${msPerToken.toFixed(2)}ms/token)`
				);
			} else {
				results.push({
					name,
...
async function measureConcurrentRequests(aiProvider, concurrency = 3) {
	...
	const promises = Array.from({ length: concurrency }, (_, index) => {
		...
		return aiProvider
			.generateObject(simplePrompt, simpleSchema, { maxTokens: 50, temperature: 0 })
			.then(result => ({
				index: index + 1,
	...
	console.log(
		`   Concurrency Efficiency: ${(avgRequestTime > 0 ? (concurrency * avgRequestTime) / totalDuration : 0).toFixed(2)}x`
	);
...
async function measureModelLatency(aiProvider, env) {
	...
	const testPrompt =
		'Extract one entity from this text: "Apple announced new iPhone." Return JSON: {"entity": "entity_name"}';
	const simpleSchema = {
		parse: obj => obj,
		safeParse: obj => ({ success: true, data: obj }),
	};
	...
	for (let i = 0; i < iterations; i++) {
		...
		try {
			const result = await aiProvider.generateObject(testPrompt, simpleSchema, {
				maxTokens: 100,
				temperature: 0.1,
			});
	...
	const latencyVariance =
		successfulResults.length > 1
			? successfulResults.reduce((sum, r) => sum + (r.duration - avgLatency) ** 2, 0) /
				(successfulResults.length - 1)
			: 0;
...
async function runAPILatencyTest() {
	...
	if (complexity) {
		...
		successfulComplexity.forEach(result => {
			console.log(
				`     ${result.name}: ${result.duration.toFixed(2)}ms (${result.msPerToken.toFixed(2)}ms/token)`
			);
		});
	}
...
function generateLatencyRecommendations(testResults) {
	...
	if (basicLatency?.avgLatency > 2000) {
		recommendations.push(
			`High API latency (${basicLatency.avgLatency.toFixed(0)}ms) - investigate network issues, API region, or provider status`
		);
	}
	...
	if (basicLatency?.successRate < 0.9) {
		recommendations.push(
			`Low API success rate (${(basicLatency.successRate * 100).toFixed(1)}%) - implement retry logic with exponential backoff`
		);
	}
	...
	if (complexity?.results) {
		...
		if (successful.length >= 2) {
			const complexityFactor =
				Math.max(...successful.map(r => r.duration)) / Math.min(...successful.map(r => r.duration));
			if (complexityFactor > 4) {
				recommendations.push(
					`High prompt complexity impact (${complexityFactor.toFixed(1)}x) - optimize prompts for production workloads`
				);
			}
		}
	}
	...
	if (concurrent?.efficiency < 0.6) {
		recommendations.push(
			`Poor concurrent efficiency (${concurrent.efficiency.toFixed(2)}x) - reduce parallel requests or implement request queuing`
		);
	} else if (concurrent?.efficiency > 0.9) {
		recommendations.push(
			`Excellent concurrent efficiency - consider increasing parallelism for better throughput`
		);
	}
	...
	if (modelLatency && modelLatency.avgLatency > 0) {
		...
		if (cv > 0.4) {
			recommendations.push(
				`High latency variability (CV: ${cv.toFixed(2)}) - implement adaptive timeout strategies`
			);
		}
	}
	...
	if (overallLatency > 5000) {
		recommendations.push(
			`Overall API performance is slow (${overallLatency.toFixed(0)}ms) - consider model switching or provider alternatives`
		);
	} else if (overallLatency > 2000) {
		recommendations.push(
			`Moderate API latency detected - monitor performance trends and implement caching where appropriate`
		);
	}
