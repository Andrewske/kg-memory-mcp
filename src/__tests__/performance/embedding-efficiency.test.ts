...
describe('Embedding Efficiency Performance', () => {
	...
	describe('Deduplication Efficiency', () => {
		it('should achieve significant deduplication in realistic datasets', async () => {
			...
			console.log(
				`Deduplication efficiency: ${(efficiency * 100).toFixed(1)}% (${stats?.duplicatesAverted}/${stats?.totalTexts} duplicates removed)`
			);
		});
	...
	describe('API Call Optimization', () => {
		it('should demonstrate 70-80% API call reduction', async () => {
			...
			console.log(
				`API call reduction: ${(reduction * 100).toFixed(1)}% (${actualApiCalls} vs ${theoreticalApiCalls} calls)`
			);
		});
	...
	describe('Memory and Performance Optimization', () => {
		it('should handle large datasets without memory issues', async () => {
			...
			console.log(
				`Large dataset processing: ${endTime - startTime}ms, ${memoryIncrease.toFixed(1)}MB memory increase`
			);
		});
		...
		it('should validate claimed 70-80% efficiency gains', async () => {
			...
			console.log(
				`Realistic dataset efficiency: ${(efficiency * 100).toFixed(1)}% (Target: 70-80%)`
			);
			console.log(
				`API calls saved: ${stats?.duplicatesAverted} out of ${stats?.totalTexts} total texts`
			);
		});
	});
});
