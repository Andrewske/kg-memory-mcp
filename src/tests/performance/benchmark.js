...
async function runBenchmark(name, filename, timeout) {
	...
	try {
		...
		const result = await Promise.race([processKnowledge(args), timeoutPromise]);
		...
		if (result.success && result.data) {
			...
			report.performance.extractionEfficiency =
				(report.results.triplesStored / estimatedTokens) * 1000;

			console.log(
				`✅ ${name}: ${(duration / 1000).toFixed(2)}s | ${report.results.triplesStored} triples | ${report.performance.tokensPerSecond.toFixed(1)} tok/s`
			);
			console.log(
				`   Efficiency: ${report.performance.extractionEfficiency.toFixed(2)} triples/1k tokens`
			);
		} else {
		...
		console.log(
			`💾 Memory: ${(memoryUsed / 1024 / 1024).toFixed(2)}MB (${(report.performance.mbPerToken * 1000).toFixed(1)}KB/token)`
		);
...
function generateReport(results) {
	...
	if (successful.length > 0) {
		...
		const avgSpeed =
			successful.reduce((sum, r) => sum + r.performance.tokensPerSecond, 0) / successful.length;
		const avgEfficiency =
			successful.reduce((sum, r) => sum + (r.performance.extractionEfficiency || 0), 0) /
			successful.length;
		const avgTimePerToken =
			successful.reduce((sum, r) => sum + r.performance.msPerToken, 0) / successful.length;
		...
		console.log(
			`Performance Range: ${fastest.testName} (${(fastest.duration / 1000).toFixed(1)}s) → ${slowest.testName} (${(slowest.duration / 1000).toFixed(1)}s)`
		);
		...
		if (avgTimePerToken > 150) {
			...
			console.log(
				`   Target after Phase 1: ~${(avgTimePerToken * 0.25).toFixed(1)}ms/token (75% faster)`
			);
			console.log('');
