...
import { createTestArgs, createMockDatabase, createMockQStash, createTestMetrics } from '../helpers/mock-factories.js';
...
describe('Pipeline Coordinator', () => {
  ...
  beforeEach(() => {
    ...
    (db as any).processingJob = mockDb.processingJob;
  ...
  describe('schedulePostProcessingJobs', () => {
    ...
    it('should calculate smart delays based on metrics', async () => {
      ...
      expect(conceptDelay).toBeGreaterThan(5); // Minimum delay
    ...
    it('should handle QStash unavailability', async () => {
      ...
      await expect(schedulePostProcessingJobs(parentJobId, metrics))
